...
myWorkspaces = [wsGEN, wsWRK, wsWRK2, wsSYS, wsMON, wsFLOAT, wsTMP]
...
myStatusBar         = "xmobar -x0 /home/dejanr/.xmonad/xmobar.conf"
...
myKeys conf = let
    ...
    , ("M-S-<Backspace>"                  , addName "Rebuild & restart XMonad"        $ spawn "xmonad --recompile && xmonad --restart")
  ...
  --, ("M-7"              , addName "TESTING"                         $ runInTerm "-name glances" "glances" )
    ...
    , ("M-S-1"                  , addName "Kill all"                        $ confirmPrompt hotPromptTheme "kill all" $ killAll)
    , ("M-<Space>"              , addName "Duplicate w to all ws"           $ toggleCopyToAll)
    , ("M-S-<Space>"            , addName "Kill other duplicates"           $ killAllOtherCopies)
    ...
    , ("M-S-2"                  , addName "Send window to Project"            $ shiftToProjectPrompt warmPromptTheme)
...
myStartupHook = do
    ...
    -- spawn "/home/dejanr/.bin/wm/init-tilingwm"
    -- spawn "/home/dejanr/.bin/wm/init-wallpaper"
